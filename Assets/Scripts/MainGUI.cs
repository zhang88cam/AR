...
//			}
			...
			secondCamera.transform.rotation = xyQuaternion;
			...
			//This is for iPad
			Vector3 p = Camera.mainCamera.ScreenToWorldPoint(new Vector3(768/2, 1024/2, 1.5f));

			target.transform.position = p;
			
			//light rotation
			light.transform.rotation = Camera.mainCamera.transform.rotation * Quaternion.AngleAxis(90.0f,Vector3.forward);
			Vector3 q = Camera.mainCamera.ScreenToWorldPoint(new Vector3(768/2, 1024/2, 1.5f));
			light.transform.position = q;
...
//			target.transform.position = Camera.mainCamera.transform.rotation * vectorDistance;
	...
	void OnGUI()
	{
		...
		if( GUI.Button( new Rect( xPos, yPos += heightPlus, width, height ), "Input Coordinates" ) )
		{
			...
			NativeToolkitBinding.activateUIWithController( "TurbineViewController" );
		}
